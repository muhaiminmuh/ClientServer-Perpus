...
router.put('/', token.cek, function (req, res, next) {
    Supplier.create(req.body, function (err, data) {
        if (err) return next(err);
        res.json({
            "status": true,
...
router.post('/', token.cek, function (req, res, next) {
    var id = req.body._id;
    Supplier.findByIdAndUpdate(id, req.body, function (err, data) {
        if (err) return next(err);
        res.json({
            "status": true,
...
router.delete('/', token.cek, function (req, res, next) {
    var id = req.body._id;
    Supplier.findByIdAndRemove(id, req.body, function (err, data) {
        if (err) return next(err);
        res.json({
            "status": true,
...
router.get('/pdf', function (req, res, next) {
    Supplier.find(function (err, data) {
        ...
        dataTampil.push(
            Array("ID", "Nama Supplier", "Alamat")
        );

        data.forEach(function (item) {
            dataTampil.push(
                Array(item.id, item.nama_supplier, item.alamat)
            );
        });


        var docDefinition = {
            header: 'Laporan Master Supplier',
            content: [{
        ...
        pdfDOc.pipe(fs.createWriteStream('pdf/lap_supplier.pdf'));
        pdfDOc.end();

        var filestream = fs.createReadStream('pdf/lap_supplier.pdf');
        filestream.pipe(res);
...
router.get('/combo', function (req, res, next) {
    ...
    Supplier.find(function (err, data) {
        ...
        data.forEach((item) => {
            ...
            pilihan.push(
                { id: item.nama_supplier, value: item.nama_supplier }
            )
        });
        res.json(pilihan);
    });
